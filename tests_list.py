...
class FunctionalTest(unittest.TestCase):
    ...
    def test_add_todo_item_answer(self):
        answer, keyboard = content_text_answer('бот добавь дело №1', '1')
    ...
    def test_delete_todo_item_answer(self):
        ...
        answer, keyboard = content_text_answer('бот удали 1', '1')
    ...
    def test_get_empty_list_answer(self):
        answer, keyboard = content_text_answer('бот покажи список', '1')
    ...
    def test_get_list_answer(self):
        ...
        answer, keyboard = content_text_answer('бот покажи список', '2')
        self.assertIn('Ваш список пока пуст', answer)

    def test_short_commands(self):
        answer, keyboard = content_text_answer('??', '1')
        self.assertEqual('Ваш список пока пуст', answer)
        answer, keyboard = content_text_answer('++ дело №1', '1')
        self.assertEqual('дело №1 добавлено', answer)
        answer, keyboard = content_text_answer('** 1', '1')
    ...
    def test_short_delete_todo_item_out_of_range(self):
        bot.data_base_lists = {'1': [{"title": 'дело №1', "done": False}]}
        answer, keyboard = content_text_answer('-- 2', '1')
        self.assertEqual('Элемента №2 нет', answer)
        answer, keyboard = content_text_answer('-- дело', '1')
    ...
    def test_error(self):
        ...
        answer, keyboard = content_text_answer('+?', '1')
        self.assertEqual('', answer)
        answer, keyboard = content_text_answer('бот добавь', '1')
        self.assertEqual('', answer)
        answer, keyboard = content_text_answer('-', '1')
        self.assertEqual('', answer)
        answer, keyboard = content_text_answer('- 1', '1')
        ...
        answer, keyboard = content_text_answer('-?', '1')
        self.assertEqual('', answer)
        answer, keyboard = content_text_answer('бот удали', '1')
        self.assertEqual('', answer)
        answer, keyboard = content_text_answer('бот', '1')
        self.assertEqual('', answer)

    def test_mark_item_answer(self):
        bot.data_base_lists = {'1': [{"title": 'дело №1', "done": False}]}
        answer, keyboard = content_text_answer('бот зачеркни 1', '1')
        self.assertEqual('дело №1 зачеркнуто', answer)
